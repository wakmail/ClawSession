...
function makeTitle(message, mode, customTitle) {
...
export async function sendMessage(cookie, message, model, orgId, titleMode = "message", customTitle) {
    const headers = {
        ...HEADERS_BASE,
        cookie: `sessionKey=${cookie}`,
    };
    const convUuid = randomUUID();
    const createResp = await fetch(`${BASE_URL}/organizations/${orgId}/chat_conversations`, {
        method: "POST",
        headers,
        body: JSON.stringify({
            uuid: convUuid,
    ...
    const completionResp = await fetch(`${BASE_URL}/organizations/${orgId}/chat_conversations/${convUuid}/completion`, {
        method: "POST",
